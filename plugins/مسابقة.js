const handler = async (m, {conn, text, args, usedPrefix, command}) => {
  ...
  const b = text.split('|');
  if (!b[1]) throw `*⚠️ الامر بيشتغل كدا ${usedPrefix + command} مثال؟|اسكونر|بوت*`;
  if (b[12]) throw `*⚠️ الامر بيشتغل كدا  ${usedPrefix + command} مثال؟|اسكونر|بوت*`;
...
handler.tags = ['group'];
handler.command = ['poll','pooll','مسابقة'];
